...
- **S3 Data Bucket**: Stores conversation events, user settings, and workspace files
...
4. Creates docker-compose.yml dynamically
5. Loads `config/config.toml` from the repo at CDK synth time
6. Starts OpenHands via systemd service
...
| Workspace Files | Local EBS | Ephemeral | Code, project files |
